...
- **Provider-based architecture**: Unified interface for multiple remote development platforms
- **Stateless design**: No local state management required, all operations are idempotent
- **Type-safe**: Full TypeScript support with comprehensive type definitions
- **Flexible configuration**: Support for custom deployment options per provider
...
```typescript
const status = await provider.getStatus('my-codespace-name');
...
```typescript
await provider.stop('my-codespace-name');
...
The Codespaces provider uses the GitHub CLI (`gh`) for authentication. Before using sudopod with Codespaces, ensure:
...
The provider will automatically validate authentication and throw helpful errors if not configured.
...
```typescript
// Codespaces provider
const codespaces = createProvider({ type: 'codespaces' });

// Coder provider (not yet implemented)
const coder = createProvider({
  type: 'coder',
...
try {
  ...
  if (error instanceof AuthenticationError) {
    ...
    console.error('Provider error:', error.operation, error.message);
...
The Codespaces provider implements a sophisticated idle timeout system:
...
5. Tests all provider methods (getStatus, list, getUrls)
...
sudopod/
├── src/
│   ├── core/           # Core provider interface and factory
