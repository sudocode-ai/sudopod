 ...
 *
 * Provider-agnostic setup operations used during workspace creation and resume.
 ...
 *
 * Used by both CodespacesProvider and CoderProvider.
 *
...
async function execOrThrow(
  ...
  if (result.exitCode !== 0) {
    ...
    throw new ExecutionError('setup', command, result.exitCode, details);
  }
  return result;
}
 ...
 *
 * Unconditional prerequisite for every workspace — runs during create()
 * regardless of whether SetupConfig is provided.
 *
 * Ensures Node.js >= 18 is available first, since default workspace
 * images may ship with an older version.
 *
 * @param workspaceName - The workspace name/identifier
 * @param exec - Function to execute commands in the workspace
 * @param workspaceDir - Optional directory to run `sudocode init` in
 */
export async function installSudocode(
  ...
  await execOrThrow(exec, workspaceName, cmd, {
    timeout: 300_000, // 5 minutes — npm install with native deps can be slow
  });
}

/**
 * Apply optional one-time setup configuration to a newly created workspace.
 *
 ...
 * @param workspaceDir - Optional working directory for service installs
 */
export async function applySetupConfig(
  ...
  if (setup.services?.length) {
    ...
    for (const svc of setup.services) {
      ...
      await exec(workspaceName, `${cdPrefix}${def.install}`);
    }
  }
...
async function joinTailnet(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      await execOrThrow(exec, workspaceName, upCmd, { timeout: 30_000 });
      return;
    } catch (err) {
      lastError = err instanceof Error ? err : new Error(String(err));
      if (attempt < maxRetries) {
        // Wait before retrying — gives daemon and network time to settle
        await exec(workspaceName, 'sleep 3');
      }
    }
  }
  throw lastError;
...
/** Default state directory — on a volume that persists across workspace stop/start. */
export const DEFAULT_STATE_DIR = '/workspaces/.tailscale';

/**
 * Start the tailscaled daemon manually (containers typically lack systemd).
...
async function startTailscaleDaemon(
  ...
  // setsid is critical for Coder (kernel mode) — coder ssh kills the entire
  // process tree on session close, so the daemon must be in a new session.
  // For Codespaces (userspace mode), plain background is sufficient and more
  // reliable — gh codespace ssh doesn't kill the process tree.
  const daemonPrefix = mode === 'kernel' ? 'setsid ' : '';

  const startAndWaitCmd =
    `${daemonPrefix}sudo tailscaled ${daemonFlags} > /tmp/tailscaled.log 2>&1 & ` +
    'for i in $(seq 1 30); do [ -S /var/run/tailscale/tailscaled.sock ] && break; sleep 0.5; done; ' +
    '[ -S /var/run/tailscale/tailscaled.sock ] && echo "tailscaled ready" || echo "tailscaled socket timeout"';

  const result = await exec(workspaceName, startAndWaitCmd, { timeout: 30_000 });

  if (!result.stdout.includes('tailscaled ready')) {
    // Socket didn't appear — read daemon log for diagnostics
    const diag = await exec(workspaceName,
      'echo "--- daemon log ---"; cat /tmp/tailscaled.log 2>&1; ' +
      'echo "--- socket dir ---"; ls -la /var/run/tailscale/ 2>&1; ' +
      'echo "--- processes ---"; ps aux | grep tailscale 2>&1',
    );
    const details = [result.stdout, result.stderr, diag.stdout].filter(Boolean).join('\n');
    throw new ExecutionError('setup', 'startTailscaleDaemon', 1, details);
  }
}

/**
 * Set up Tailscale in a workspace using progressive detection.
 *
 ...
 * The default workspace image gets the full install (tier 3).
 *
 * @param workspaceName - Workspace name (also used as tailnet hostname)
 * @param exec - Function to execute commands in the workspace
...
export async function setupTailscale(
  ...
  const upCmd = buildTailscaleUpArgs(workspaceName, config);

  // Probe 1: Is tailscale installed?
  const whichResult = await exec(workspaceName, 'which tailscale');
  ...
  const statusResult = await exec(workspaceName, 'sudo tailscale status 2>&1', {
    timeout: 10_000,
  });
